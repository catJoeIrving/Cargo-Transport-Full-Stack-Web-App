...
def delete_captain_record():
    ...
    idToDelete = int(request.args['secondary_id'])
    for i in range(len(captain) -1, -1, -1):
        if captain[i]['secondary_id'] == idToDelete:
...
def usernamepw_example():
    ...
    for au in authorized_users: #loop over all users and find one that is authorized to access
        if au['username'] == username and au['password'] == pw: #found an authorized user
            print("You're authorized to see this but now we have to figure out what you want to do")

    return 'The police are on their way' # Authentication failed
app.run()
